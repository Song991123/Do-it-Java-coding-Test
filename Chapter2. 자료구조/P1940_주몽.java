...
public class P1940_주몽 {
    public static void main(String[] args) throws IOException{
        // N이 15,000이므로 sort하기가 편함. 투 포인트는 O(n)이지만 sort가 O(nlogn)이므로 O(nlogn)
        // =======================
        // 슈도 코드
        // =======================
        // N(재료의 개수), M(갑옷 만드는 데 필요한 수) 입력
        // for(N만큼 반복) { 재료 번호를 저장하는 배열 저장}
        // 재료 번호를 저장하는 배열 정렬
        // start = 0, end = N-1, count = 0 으로 변수 선언 및 초기화
        // while(start < end){
        //      if(재료 합 > M) end--
        //      else if (재료 합 < M) start++
        //      else if (재료 합 == M) end--, start++, count++
        // }
        // count 출력
        //
        ...
        while(start < end){
            if(A[start] + A[end] > M) end--;
            else if(A[start] + A[end] < M) start++;
            else{
                start++; end--; count++;
            } 
        }
